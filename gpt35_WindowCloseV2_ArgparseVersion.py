...
def produce_initial_response(logger, initial_state_info, b_instruction, open_ai_api_key, gpt = "gpt-3.5-turbo", temperature = 0.3):
    with open('llm/system_prompt_MetaWorldWindowCloseV2_0411.txt', 'r') as file:
...
# should be done modified.
# 20240328, done
def produce_feedback_response(logger, current_state_info, b_instruction, reward_feedback, open_ai_api_key, gpt = "gpt-3.5-turbo", temperature = 0.3):
    with open('llm/system_prompt_MetaWorldWindowCloseV2_0411.txt', 'r') as file:
        system_prompt_contents = file.read()
...
if __name__ == "__main__":
    ...
    while True:
        action = produce_initial_response(logger, init_obs_info_dict, str(selected_instruction), config['api_key'], gpt = "gpt-3.5-turbo", temperature = 0.3)
        if (action != []):
            break
    ...
    while ((not done) and (cnt <= 1000) and (not succeeded)):
        ...
        while (not action_valid):
            further_action = produce_feedback_response(logger, curr_obs_info_dict, str(selected_instruction), str(feedback_with_reward), config['api_key'], gpt = "gpt-3.5-turbo", temperature = 0.3)
            if (further_action != []):
                action_valid = True
    ...
    # Saving micro
    np.savez(f'./outdir_{ENV_NAME}/WindowCloseV2_4traj_seed{SEED_NUMBER}_idx{task_idx}_micro.npz', all_traj=global_traj_list_micro, all_states=all_s_micro, all_actions=all_act_micro, all_rewards=all_r_micro, global_step_cnt=sum(global_step_count))
