...
class MetaworldWrapper(LLFWrapper):
    ...
    def _step(self, action):
        ...
        # record the micro (obs, action, reward)
        traj_list = []
        # print("self.p_control_time_out : ", self.p_control_time_out)
        for _ in range(self.p_control_time_out):
            ...
            # 20240314, these obs, reward, control are the "micro" obs, reward, action that we need for the RL agent to follow.
            traj_list.append(control)
            traj_list.append(reward)
            traj_list.append(observation)
        ...
        # 20240314, we don't want LLMs to see information other than the same info provided to the RL agent.
        # Hence, additional feedbacks need to be commented.
