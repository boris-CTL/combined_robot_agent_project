...
for SEED_NUMBER in seed_lists:
    for task_idx in range(task_per_seed):
        ...
        loaded_data = np.load(f'../basketball_v2_trajectories/BasketballV2_4traj_seed{SEED_NUMBER}_idx{task_idx}_macro.npz', allow_pickle=True)
...
# Save the dataset
np.savez('basketball_expert_dataset.npz', expert_observations=expert_observations, expert_actions=expert_actions)