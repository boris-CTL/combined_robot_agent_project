...
import tqdm as tqml
from llm.openai_interact import init as llm_init
from llm.openai_interact import produce_program as llm_produce_program
from llm.openai_interact import produce_feedback_programs as llm_produce_feedback_programs
...
def collect_program_rollouts(config, logger, model, trajectories_str, program_karel_world, program_state_generator, reflection=False, previous_programs_str="", baseline=0.0):
    ...
    if reflection:
        ...
        programs = llm_produce_feedback_programs(logger, task_desc=task_desc, program_num=config['llm_program_num'], init_state_str=init_state_str, trajectories_str=trajectories_str, previous_programs_str=previous_programs_str, gpt=config['gpt'], temperature=config['llm_temperature'])
    else:
        programs = llm_produce_program(logger, task_desc=task_desc, program_num=config['llm_program_num'], init_state_str=init_state_str, gpt=config['gpt'], temperature=config['llm_temperature'])
    ...
    #     total_timesteps=int(config['offline_timesteps']),
    # )

    # callback.on_training_start(locals(), globals())

    model.learn(total_timesteps=0, reset_num_timesteps=False, callback=WandbCallback())
    
    # show progress bar
    for timestep in tqml.trange(config['offline_timesteps'], desc="Offline Training", unit=" timesteps"):
...
def run(config, logger):
    ...
    model = eval(config['algorithm'])(
        ...
        learning_starts=0 if config['collect_program_rollouts'] else 50000, 
        exploration_initial_eps= 0.5, # if config['collect_program_rollouts'] else 1.0, 
    ...
    if config['collect_program_rollouts']:
        ...
        programs, programs_reward_list, previous_programs_str = collect_program_rollouts(config, logger, model, None, karel_world, state_generator, reflection=False)
        ...
        # collect a trajectory from the policy
        # logger.debug('Collecting trajectory from policy')
        # obs, info = env.reset()
        # for i in range(10): #config['max_episode_steps']):
        #     # action, _states = model.predict(obs, deterministic=True)
        #     # log the model q values
        #     observation, vectorized_env = model.policy.obs_to_tensor(obs)
        #     with torch.no_grad():
        #         q_values = model.policy.q_net(observation)
        #     logger.debug('Q values - step {}: {}'.format(i, q_values))
        #     action = q_values.argmax(dim=1).reshape(-1).cpu().numpy().reshape((-1, *model.action_space.shape))
        #     obs, rewards, terminated, truncated, info = env.step(action)
        #     # env.render()
        #     # save the trajectory
        #     if terminated or truncated or i == config['max_episode_steps'] - 1:
        #         s_h = env.render()
        #         save_video_path = os.path.join(config['outdir'], 'programs', '{}_eval.gif'.format(config['env_task']))
        #         save_gif(karel_world, save_video_path, s_h)
    ...
    for i in range(epoch):
        ...
        programs, programs_reward_list, previous_programs_str = collect_program_rollouts(config, logger, model, trajectories, karel_world, state_generator, reflection=True, previous_programs_str=previous_programs_str, baseline=eval_results[0])
